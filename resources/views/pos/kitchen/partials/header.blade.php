<div class="h-14 sm:h-16 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-3 sm:px-6 shrink-0 z-20 shadow-md gap-2 transition-colors duration-300">
    ...
    {{-- CENTER --}}
    <div class="flex bg-gray-100 dark:bg-gray-900 p-1 rounded-lg border border-gray-200 dark:border-gray-700 shrink-0 overflow-x-auto custom-scrollbar max-w-[45%] sm:max-w-md">
        <div class="flex gap-1">
            @foreach($destinations as $dest)
                <button @click="changeMode({{ $dest->id }})" 
                        class="px-3 sm:px-6 py-1.5 sm:py-2 rounded-md text-xs sm:text-sm font-bold transition-all flex items-center gap-2 whitespace-nowrap"
    ...
    <div class="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        <div class="flex items-center gap-1 sm:gap-2 mr-2">
            ...
            <span class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 font-mono hidden sm:inline" x-text="isLoading ? '{{ __('messages.fetching') }}' : '{{ __('messages.live') }}'"></span>
        </div>
        ...
        {{-- Theme & Profile --}}
        {{-- (កូដ Theme & Profile រក្សាទុកនៅដដែលបាន) --}}
        <button x-data="{ 
