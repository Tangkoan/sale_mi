...
<div class="h-screen w-full bg-[#F6F8FC] dark:bg-[#0f172a] flex flex-col relative overflow-hidden font-sans" x-data="posMenu()" x-cloak>
    ...
    <div class="z-30 shrink-0 bg-white/80 dark:bg-gray-800/90 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 shadow-sm sticky top-0">
        <div class="px-3 py-2 sm:px-4 sm:py-3 flex items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2 sm:gap-3 overflow-hidden flex-1 min-w-0">
                <a href="{{ route('pos.tables') }}" class="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 font-bold hover:bg-gray-200 transition flex items-center justify-center sm:px-4 sm:justify-start gap-2">
            ...
            <div class="flex items-center gap-2">
                ...
                {{-- Kitchen Link --}}
                <a href="{{ route('pos.kitchen.view') }}" target="_blank" class="hidden sm:flex items-center justify-center w-10 h-10 rounded-full bg-orange-100 text-orange-600 hover:bg-orange-200 transition" title="Kitchen Screen">
            ...
            <template x-for="product in filteredProducts" :key="product.id">
                <div @click="product.is_active ? openProductModal(product) : null" 
                    ...
                    <div x-show="!product.is_active" class="absolute inset-0 z-10 flex items-center justify-center bg-gray-900/10 backdrop-blur-[1px]">
                        <span class="bg-red-500 text-white text-[10px] sm:text-xs font-bold px-2 sm:px-3 py-1 rounded-full shadow-lg transform -rotate-12 border-2 border-white">
                            OUT OF STOCK
                        </span>
                    </div>
                    ...
                    <div class="px-0.5 sm:px-1">
                        <h3 class="font-bold text-gray-800 dark:text-gray-200 text-xs sm:text-sm leading-snug line-clamp-2 min-h-[2.5em]" x-text="product.name"></h3>
                        <div class="mt-1 sm:mt-2 flex items-center justify-between">
                            <span class="font-black text-sm sm:text-base" :class="product.is_active ? 'text-primary' : 'text-gray-500'" x-text="'$' + parseFloat(product.price).toFixed(2)"></span>
                        </div>
                    </div>
                </div>
            </template>
    ...
    <div x-show="isProductModalOpen" class="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4" style="display: none;" x-cloak>
        ...
        {{-- Responsive Modal --}}
        <div class="relative w-full sm:max-w-md bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] sm:max-h-[85vh] transition-all" 
            ...
            {{-- Image Header --}}
            <div class="relative h-48 sm:h-56 bg-gray-100 dark:bg-gray-700 shrink-0">
                <template x-if="tempItem.image"><img :src="'/storage/' + tempItem.image" class="w-full h-full object-cover"></template>
                <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                <button @click="closeProductModal()" class="absolute top-3 right-3 sm:top-4 sm:right-4 bg-black/20 text-white rounded-full p-1.5 sm:p-2 hover:bg-black/40 backdrop-blur-md"><i class="ri-close-line text-xl sm:text-2xl"></i></button>
                <div class="absolute bottom-3 left-4 right-4 sm:bottom-4 sm:left-6 sm:right-6 text-white"><h2 class="text-xl sm:text-2xl font-bold leading-tight shadow-black drop-shadow-md" x-text="tempItem.name"></h2></div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-4 sm:p-6 custom-scrollbar">
                ...
                <div class="flex items-center justify-between mb-6 sm:mb-8">
                    <div>
                        <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 font-medium">Base Price</p>
                        <p class="text-2xl sm:text-3xl font-black text-primary" x-text="'$' + parseFloat(tempItem.base_price).toFixed(2)"></p>
                    </div>
                    <div class="flex items-center gap-3 sm:gap-4 bg-gray-100 dark:bg-gray-700 rounded-xl sm:rounded-2xl p-1.5 shadow-inner">
                        <button @click="if(tempItem.qty > 1) tempItem.qty--" class="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-white text-gray-800 hover:text-primary"><i class="ri-subtract-line font-bold"></i></button>
                        <span class="font-bold text-lg sm:text-xl min-w-[1.2em] sm:min-w-[1.5em] text-center text-gray-800 dark:text-white" x-text="tempItem.qty"></span>
                        <button @click="tempItem.qty++" class="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-white text-gray-800 hover:text-primary"><i class="ri-add-line font-bold"></i></button>
                    </div>
                </div>

                {{-- Addons List --}}
                <div class="mb-4 sm:mb-6">
                    <h3 class="text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 sm:mb-3">{{ __('messages.addon_list') }}</h3>
                    <div class="space-y-2 sm:space-y-2.5">
                        <template x-for="addon in availableAddons" :key="addon.id">
                            <div class="flex items-center justify-between p-2.5 sm:p-3 rounded-xl sm:rounded-2xl border transition-all duration-200" :class="isAddonSelected(addon.id) ? 'border-primary bg-primary/5 shadow-inner' : 'border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'">
                                <div class="flex items-center gap-2 sm:gap-3 cursor-pointer flex-1" @click="toggleAddon(addon)">
                                    <div class="relative flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 rounded border transition-colors shrink-0" :class="isAddonSelected(addon.id) ? 'bg-primary border-primary' : 'border-gray-300 bg-white'">
                                        <i class="ri-check-line text-white text-xs sm:text-sm" x-show="isAddonSelected(addon.id)"></i>
                                    </div>
                                    <div>
                                        <p class="text-xs sm:text-sm font-bold text-gray-800 dark:text-gray-200" x-text="addon.name"></p>
                                        <p class="text-[10px] sm:text-xs text-primary font-bold" x-text="'+$' + parseFloat(addon.price).toFixed(2)"></p>
                                    </div>
                                </div>
                                <div x-show="isAddonSelected(addon.id)" x-transition class="flex items-center bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-600 h-7 sm:h-8 ml-2">
                                    <button @click.stop="updateAddonQty(addon.id, -1)" class="px-1.5 sm:px-2 text-gray-500 hover:text-red-500"><i class="ri-subtract-line text-xs sm:text-sm"></i></button>
                                    <span class="text-xs sm:text-sm font-bold w-5 sm:w-6 text-center text-gray-800 dark:text-white" x-text="getAddonQty(addon.id)"></span>
                                    <button @click.stop="updateAddonQty(addon.id, 1)" class="px-1.5 sm:px-2 text-gray-500 hover:text-primary"><i class="ri-add-line text-xs sm:text-sm"></i></button>
                                </div>
                            </div>
                        </template>
                        <div x-show="availableAddons.length === 0" class="text-xs sm:text-sm text-gray-400 italic text-center py-2">No addons available.</div>
                    </div>
                </div>

                {{-- Note --}}
                <div class="mb-2">
                    <label class="block text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-widest mb-1.5 sm:mb-2">Note</label>
                    <textarea x-model="tempItem.note" rows="2" class="w-full px-3 py-2 sm:px-4 sm:py-3 rounded-xl sm:rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 focus:ring-2 focus:ring-primary/50 outline-none text-xs sm:text-sm" placeholder="Less sugar, no spicy..."></textarea>
                </div>
            </div>

            <div class="p-3 sm:p-4 border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 pb-6 sm:pb-4 shadow-[0_-5px_15px_rgba(0,0,0,0.02)] z-20">
                <button @click="addToCart()" class="w-full bg-primary text-white py-3 sm:py-4 rounded-xl sm:rounded-2xl font-bold text-base sm:text-lg hover:brightness-110 active:scale-[0.98] transition-all flex justify-between px-4 sm:px-6 shadow-lg shadow-primary/30 relative overflow-hidden">
...
<script>
    ...
    // The JavaScript logic remains exactly the same as in your previous version.
    // Just copy the <script> block from your previous request here.
    function posMenu() {
        return {
