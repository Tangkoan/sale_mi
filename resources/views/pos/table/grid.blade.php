    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-6">
        <template x-for="table in tables" :key="table.id">
            <div class="relative group">
                ...
                {{-- Quick Checkout Button (កែសម្រួលសម្រាប់ Mobile) --}}
                <template x-if="table.status === 'busy'">
                    <button @click.prevent="openQuickCheckout(table)"
                            class="absolute top-2 right-2 sm:top-2 sm:right-2 w-10 h-10 sm:w-12 sm:h-12 bg-white dark:bg-gray-700 text-rose-500 rounded-full shadow-lg hover:bg-rose-500 hover:text-white hover:scale-110 active:scale-90 transition-all flex items-center justify-center border border-rose-100 dark:border-rose-900 z-20"
