<div x-show="isModalOpen" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center px-4" x-cloak>
    ...
    <div class="relative w-full max-w-md bg-card-bg rounded-2xl shadow-2xl border border-border-color overflow-hidden"
        ...
        <div class="px-6 py-4 border-b border-border-color flex justify-between items-center" :class="isSequenceMode ? 'bg-blue-50 dark:bg-blue-900/20' : 'bg-page-bg/30'">
            <div>
                {{-- Translate Title based on Logic --}}
                <h3 class="text-lg font-bold text-text-color" x-text="editMode ? '{{ __('messages.edit_destination') }}' : '{{ __('messages.add_destination') }}'"></h3>
                <template x-if="isSequenceMode">
                    <p class="text-xs text-primary font-bold mt-1">
                        {{ __('messages.editing') }} <span x-text="currentSeqIndex + 1"></span> {{ __('messages.of') }} <span x-text="sequenceQueue.length"></span>
                    </p>
                </template>
            </div>
        ...
        <form @submit.prevent="submitForm" class="p-6 space-y-5">
            {{-- Name --}}
            <div>
                <label class="block text-sm font-bold text-text-color mb-1">{{ __('messages.destination_name') }}</label>
                <input type="text" x-model="form.name" placeholder="{{ __('messages.name_placeholder') }}" class="w-full px-4 py-2.5 rounded-lg border border-input-border bg-input-bg text-text-color focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none">
                <p x-show="errors.name" x-text="errors.name" class="text-red-500 text-xs mt-1"></p>
            </div>

            {{-- PrintNode ID --}}
            <div>
                <label class="block text-sm font-bold text-text-color mb-1">{{ __('messages.printnode_id') }}</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-secondary">
                        <i class="ri-qr-code-line"></i>
                    </span>
                    <input type="text" x-model="form.printnode_id" placeholder="{{ __('messages.id_placeholder') }}" class="w-full pl-10 pr-4 py-2.5 rounded-lg border border-input-border bg-input-bg text-text-color focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none font-mono">
                </div>
            ...
            <div class="pt-4 flex justify-between items-center border-t border-border-color mt-2">
                ...
                <div class="flex gap-3">
                    ...
                    <button type="submit" class="bg-primary text-white px-6 py-2 rounded-lg hover:opacity-90 transition flex items-center gap-2 shadow-lg shadow-primary/30" :disabled="isLoading">
                        ...
                        {{-- Logic for Button Text translated --}}
