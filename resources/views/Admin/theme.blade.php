...
<div class="max-w-5xl mx-auto pb-32" 
    ...
    <div class="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
        ...
        <div class="bg-gray-100 dark:bg-gray-800 p-1.5 rounded-xl flex shadow-inner">
            <button @click="activeTab = 'light'; $store.theme.setMode('light')" 
            ...
            <button @click="activeTab = 'dark'; $store.theme.setMode('dark')" 
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6" :key="activeTab">
        <div class="bg-card-bg border border-border-color rounded-2xl p-6 shadow-custom transition-all duration-300">
            ...
            @include('components.color-input', ['label' => 'Primary Color', 'key' => 'primary'])
    ...
    <div class="fixed bottom-6 left-6 right-6 md:left-80 md:right-10 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md border border-gray-200 dark:border-gray-700 p-4 rounded-2xl shadow-2xl z-50 flex items-center justify-between transform transition-all hover:scale-[1.01]">
        ...
        <div class="flex gap-3">
            <button @click="$store.theme.reset()" 
                    class="px-5 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 font-bold hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                Reset
            </button>
            
            <button @click="$store.theme.save()" 
                    :disabled="$store.theme.isSaving"
                    :class="$store.theme.isSaving ? 'opacity-75 cursor-not-allowed' : 'hover:shadow-lg hover:-translate-y-0.5'"
                    class="px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold shadow-md transition-all flex items-center gap-2">
                
                <i x-show="!$store.theme.isSaving" class="ri-save-3-fill text-lg"></i>
                <svg x-show="$store.theme.isSaving" class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
