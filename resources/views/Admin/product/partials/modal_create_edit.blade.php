<div x-show="isModalOpen" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center px-4" x-cloak>
    ...
    <div class="relative w-full max-w-2xl bg-card-bg rounded-2xl shadow-2xl border border-border-color overflow-hidden flex flex-col max-h-[90vh]"
        ...
        <div class="px-6 py-4 border-b border-border-color flex justify-between items-center bg-page-bg/30">
            <div>
                <h3 class="text-lg font-bold text-text-color" x-text="editMode ? '{{ __('messages.edit') }}' : '{{ __('messages.create') }}'"></h3>
        ...
        <form @submit.prevent="submitForm" class="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                ...
                <div class="relative" x-data="categoryDropdown()" @click.outside="closeDropdown()">
                    ...
                    <div x-show="isOpen" x-transition.opacity.duration.200ms class="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-border-color rounded-lg shadow-xl max-h-60 overflow-hidden flex flex-col">
                        <ul class="overflow-y-auto custom-scrollbar flex-1 p-1">
                            <template x-if="isLoading && page === 1"><li class="px-4 py-3 text-center text-sm text-secondary"><i class="ri-loader-4-line animate-spin"></i> {{ __('messages.loading') }}</li></template>
                        ...
                        <div x-show="hasMorePages" class="border-t border-border-color p-2 bg-gray-50 dark:bg-gray-900/50 text-center shrink-0">
                            <button type="button" @click="loadMore()" class="text-xs font-bold text-primary hover:underline flex items-center justify-center gap-1 w-full py-1">
                                <span x-show="!isLoading">{{ __('messages.load_more') }} <i class="ri-arrow-down-s-line"></i></span>
                                <span x-show="isLoading"><i class="ri-loader-4-line animate-spin"></i> {{ __('messages.loading') }}</span>
                            </button>
                        </div>
                    </div>
            ...
            <div class="border-t border-border-color pt-4" x-show="visibleAddons.length > 0">
                <div class="flex justify-between items-center mb-2">
                    <label class="block text-sm font-bold text-text-color">{{ __('messages.available_addons') }}</label>
                    <label class="flex items-center gap-2 cursor-pointer text-xs text-primary font-bold hover:text-primary/80">
                        <input type="checkbox" x-model="selectAllAddons" @change="toggleSelectAllAddons()" class="rounded border-input-border text-primary focus:ring-primary h-3.5 w-3.5"> {{ __('messages.select_all') }}
                    </label>
                </div>
                    ...
                    <template x-for="addon in visibleAddons" :key="addon.id">
                        <label class="flex items-center gap-2 cursor-pointer hover:bg-white dark:hover:bg-white/5 p-2 rounded transition-colors border border-transparent hover:border-input-border shadow-sm bg-card-bg">
                            ...
                            <div class="text-xs">
                                ...
                                <span class="text-secondary font-semibold" x-text="'+ ' + parseFloat(addon.price).toLocaleString() + ' ៛'"></span>
                            </div>
                        </label>
                    </template>
                </div>
            ...
            <div class="border-t border-border-color pt-4 text-center text-sm text-secondary italic" x-show="!form.category_id">{{ __('messages.select_category_first') }}</div>
            <div class="border-t border-border-color pt-4 text-center text-sm text-secondary italic" x-show="form.category_id && visibleAddons.length === 0">{{ __('messages.no_addons_category') }}</div>
        </form>

        {{-- Modal Footer --}}
        <div class="p-6 pt-0 flex justify-between items-center border-t border-border-color mt-auto bg-card-bg z-10 pt-4">
            <button type="button" x-show="isSequenceMode" @click="nextInSequence()" class="text-secondary hover:text-text-color text-sm font-bold px-2">{{ __('messages.skip_this') }} <i class="ri-arrow-right-line align-middle"></i></button>
            <div x-show="!isSequenceMode"></div> 
            <div class="flex gap-3">
                <button type="button" @click="closeModal(true)" class="px-4 py-2 rounded-lg border border-input-border text-text-color hover:bg-page-bg transition font-bold">{{ __('messages.cancel') }}</button>
