<div class="flex flex-col gap-3">
    ...
    <div class="flex items-center justify-between px-2" x-show="products.length > 0">
        ...
        <span class="text-xs text-secondary"><span x-text="products.length"></span> {{ __('messages.items') }}</span>
    </div>
                ...
                <div class="flex-1 min-w-0 flex flex-col justify-between py-0.5" :class="{'pl-8': !showCols.image}">
                    ...
                    <div class="flex items-center justify-between mt-3 pt-2 border-t border-dashed border-border-color">
                        <div class="flex items-center gap-2">
                            ...
                            @else
                            <span class="w-2 h-2 rounded-full" :class="item.is_active ? 'bg-green-500' : 'bg-gray-400'"></span>
                            @endcan
                            <span class="text-[10px] font-bold uppercase" :class="item.is_active ? 'text-green-600' : 'text-gray-400'" x-text="item.is_active ? '{{ __('messages.active') }}' : '{{ __('messages.inactive') }}'"></span>
                        </div>
