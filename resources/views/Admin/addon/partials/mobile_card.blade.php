<div class="flex flex-col gap-3">
    ...
    <div class="flex items-center justify-between px-2" x-show="addons.length > 0">
        ...
        <span class="text-xs text-secondary">
            <span x-text="addons.length"></span> {{ __('messages.items') }}
        </span>
    </div>
    ...
    <template x-for="item in addons" :key="'mobile-' + item.id">
        <div class="bg-card-bg p-4 rounded-2xl shadow-sm border border-border-color relative overflow-hidden transition-all duration-200"
            ...
            <div class="pl-8 flex flex-col gap-2">
                ...
                {{-- 2. Destination Row --}}
                <div class="flex flex-wrap items-center gap-2" x-show="showCols.destination">
                    ...
                    <template x-if="!item.destination">
                        <span class="text-xs text-gray-400 italic">{{ __('messages.no_destination') }}</span>
                    </template>
                </div>
                    ...
                    <div x-show="showCols.status">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[10px] font-bold uppercase" 
                                  ...
                                  x-text="(item.is_active == 1 || item.is_active == true) ? '{{ __('messages.active') }}' : '{{ __('messages.inactive') }}'"></span>
                        </div>
                    </div>
